...
.controller('WorkoutController', ['$scope', '$interval', '$location', 'workoutHistoryTracker', 'appEvents', '$routeParams', 'WorkoutService', 'Exercise', 
  function ($scope, $interval, $location, workoutHistoryTracker, appEvents, $routeParams, WorkoutService, Exercise) {
    ...
    if (event.which == 80 || event.which == 112) { // p or P pressed
  ...
  // Utilisation des promises plutôt que des watch
...
angular.module('7MinWorkout').controller('WorkoutAudioController', ['$scope', '$interval', '$location', '$timeout', WorkoutAudioController])

function WorkoutAudioController ($scope, $interval, $location, $timeout) {
